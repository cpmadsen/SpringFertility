...
location_costs <- data.frame(
  ...
  euploid    = c(21689,   25880,   18601),
  live_birth = c(31555,   36630,   25742),
...
stopifnot(
  "T_cumulative < T1 in some profiles -- check theta" =
    all(egg2_grid$Threshold_2cy_Cumulative_90pct >=
          egg2_grid$Threshold_1cy_90pct)
)
message("    Diagnostic [1] PASSED: T_cumulative >= T1 for all profiles.")

n_exceeds_2x <- sum(
  egg2_grid$Threshold_2cy_Cumulative_90pct >
    egg2_grid$Threshold_1cy_x2_UpperBound
)
if (n_exceeds_2x > 0) {
  message(sprintf(paste0(
    "    Diagnostic [1] NOTE: %d profiles have T_cumulative > 2*floor(T1). ",
    "Expected due to flooring of T1 before doubling -- not an error."
  ), n_exceeds_2x))
}
...
diagnostic_profiles <- egg2_grid %>%
  ...
  dplyr::select(
    Age, AMH, AFC,
    Expected_Eggs_1cy,
    Expected_Eggs_2cy_Cumulative,
    Threshold_1cy_90pct,
    Threshold_2cy_Cumulative_90pct,
    Threshold_Increment
  ) %>%
  as.data.frame()

print(diagnostic_profiles, row.names = FALSE)

# [4] Training data calibration check
df_egg1$mu_fitted <- predict(egg1_model, type = "response")
...
if (abs(observed_fail_rate - 0.10) > 0.03) {
  warning(sprintf(paste0(
    "\nEgg1 calibration check: observed fail rate = %.4f, expected 0.10.\n",
    "Deviation exceeds 0.03 threshold.\n",
    "Consider mu calibration before computing thresholds.\n",
    "Flag to model lead before proceeding to production."
  ), observed_fail_rate))
} else {
  message("    Diagnostic [4] PASSED: observed fail rate within 0.03 of 0.10.")
}
...
writeData(wb, "Model Coefficients",
          paste0("Generated by fertility_master_v7.R. ",
                 ...
                 "(NegBin convolution: T_cum = qnbinom(0.10, theta, 2*mu))."),
...
egg_models_out <- list(
  ...
  list(name="EGG 2-CYCLE MODEL (CUMULATIVE)", start_col=14, cy=2,
       ...
       note=paste0(
         ...
         ", mu=2*mu). Independent cycles assumed."))
)
...
for (em in egg_models_out) {
  ...
  # mu formula: egg1 coefficients for both panels
  # Egg2 panel shows 2*mu (cumulative expected eggs)
  base_mu_formula <- paste0(
    "EXP(",
    round(e1_coefs["(Intercept)"],8), "+",
    round(e1_coefs["age_model"],8),    "*C11+",
    round(e1_coefs["amh"],8),          "*C12+",
  ...
  if (em$cy == 1) {
    ...
    mu_label   <- "Expected eggs per cycle (mu)"
  } else {
    mu_formula <- paste0("=2*", base_mu_formula)
    mu_label   <- "Expected eggs cumulative 2 cycles (2*mu)"
  }
...
pool_stats_df <- data.frame(
  ...
  Value = c(
    ...
    "NegBin convolution: qnbinom(0.10, theta, 2*mu)",
