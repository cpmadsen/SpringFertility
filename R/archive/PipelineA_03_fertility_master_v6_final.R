...
# it is no longer read from eu_cal_int_refit.rds / eu_cal_slope_refit.rds.
#
...
# Euploid calibration hardcoded to validation-derived values (not read from RDS).
...
#   Live birth : birth_bool ~ age_model + amh + afc + age_model:amh
...
livebirth_model <- glm(
  # birth_bool ~ age_model * amh * afc,               # prior full 3-way spec
  # birth_bool ~ age_model + amh + afc + age_model:afc,  # prior age:afc spec
  birth_bool ~ age_model + amh + afc + age_model:amh,
  data   = df_livebirth,
  family = binomial
)
# Coefficient order (5 terms):
# B2=intercept B3=age_model B4=amh B5=afc B6=age_model:amh
# Prior 3-way spec had 8 terms:
...
writeData(wb, "Model Coefficients",
          paste0("Generated by fertility_master_v6_final.R. ",
                 "Coefficients use age_model (floored at 32), amh (capped at 12), ",
...
model_names_list <- list(
  ...
  list(name="LIVE BIRTH MODEL", coefs=lb_coefs,
       ...
       note="age_model:amh interaction term"),
  ...
  list(name="LIVE BIRTH MODEL",
       ...
       p_raw_formula=paste0(
         "=1/(1+EXP(-(", paste(
           ...
           # Active interaction is age_model:amh (cells C11*C12).
           paste0(round(lb_coefs["age_model:amh"],8),"*C11*C12"),
           # Terms below not in the current live-birth spec:
