...
```{r}
# # Bin the AMH values.
# pregnancy_sim_results = pregnancy_sim_results |> 
#   dplyr::mutate(amh_bin = dplyr::case_when(
#       amh < 0.5 ~ "Very low",
#       amh >= 0.5 & amh < 1.0 ~ "Low",
...
#     )
#   ) |> 
#   dplyr::mutate(amh_bin = factor(amh_bin, levels = c("Very low","Low","Normal","High","Unknown")))

# Uses simulated successes or failures
# p1_df = pregnancy_sim_results |>
#   dplyr::mutate(failure_sim = ifelse(success_sim == 1, 0, 1)) |>
#   dplyr::mutate(payout_amt = failure_sim * 20000) |>  # Arbitrary amount of payout!
#   dplyr::group_by(age) |>
#   dplyr::reframe(mean_sim_payout = mean(payout_amt, na.rm=T),
#                  total_sim_payout = sum(payout_amt, na.rm=T),
#                  sd_sim_payout = sd(payout_amt, na.rm=T),
#                  number_patients = dplyr::n()) |>
#   dplyr::mutate(mean_payout_per_person = total_sim_payout / number_patients)
# 
# p1_df |> 
#   ggplot(aes(x = age, y = mean_sim_payout)) +
#   geom_smooth() +
...
p1_df = pregnancy_sim_results |> 
  ...
  dplyr::reframe(mean_pred_payout = mean(pred_payout, na.rm=T),
                 sd_pred_payout = sd(pred_payout,na.rm=T),
                 number_patients = dplyr::n())
...
p1_df |> 
  ...
  # facet_wrap( ~ amh_bin, ncol = 1) +
  labs(y = "Total Simulated Payout by Age", colour = 'AMH bin', x = "Age",
       title = "Clinical Pregnancy Predicted Average Payout") +
  scale_y_continuous(labels = scales::dollar) +
  scale_x_continuous(limits = c(30,42), breaks = c(30:42), labels = c(30:42)) + 
  theme_minimal()
```
