...
# data.frame(b = s_egg_p) |> 
#   ggplot() + geom_histogram(aes(b))
...
s_egg_sim_results |> 
  dplyr::group_by(age_group) |> 
  dplyr::reframe(
    prob_seven_eggs_or_more_sim = mean(prob_seven_eggs_or_more, na.rm = TRUE)
  ) |> 
  dplyr::arrange(age_group) |> 
  knitr::kable()
```
...
df_multiple = df_multiple |> 
  dplyr::group_by(pat_id, treat_course_id) |>
  # Only include the first two rows in any given treatment course ID.
  dplyr::reframe(
