...
patient_summary <- m1df %>%
  group_by(pat_id, age, age_group, amh, afc) %>%
  summarize(
    attempts = n(),
...
binom_mod = run_binomial_model_with_various_distributions(patient_summary, formula = "cbind(successes, attempts - successes) ~ age + amh + afc") |> 
  suppressMessages()
...
euploid_summary <- egg_df |>
  ...
  dplyr::reframe(
    ...
    mean_afc = mean(afc_first_baseline,na.rm=T),
    mean_amh = mean(amh,na.rm=T)
  )
...
euploid_binom_model <- glm(
  cbind(successes, number_of_eggs_attempted - successes) ~ age + mean_afc + mean_amh,
