...
apply_binomial_models_in_succession(dat){
...
egg_df <- egg_df %>%
  # Drop rows where cryo_day is 2 or 3.
...
These counts consider a treatment cycle to be TRUE for frozen_egg or frozen_embryo if one or more frozen egg or embryo was used in the treatment (otherwise it's difficult to know how to count treatment cycles that may have some eggs that are frozen and some are not - if that occurs).
```{r}
df |> 
  dplyr::count(frozen_egg, frozen_embryo) |> 
  knitr::kable()

df |> 
  dplyr::count(num_2pn_from_frozen_eggs) |> 
  dplyr::mutate(total_from_frozen_eggs = sum(n[num_2pn_from_frozen_eggs != 0])) |> 
  knitr::kable()
...
# Recalculate frozen_egg / frozen_embryo as we did above.
egg_df = egg_df |> 
  dplyr::mutate(
    frozen_egg = cryo_day_0
  ) |> 
...
# validate_model(euploid_bb, euploid_summary, "successes")
...
euploid_sim_results |> 
  summarise_to_age_classes('simulated_p')
```
