...
attempt_predictions <- purrr::map_dfr(cycle_seq, function(cyc) {
  purrr::map_dfr(age_seq, function(a) {
    
    newdata <- df_attempt_recalc %>%
      select(afc, amh_bin) %>%
      mutate(
        age = a,
        attempt_num = cyc)
    newdata$predicted_p <- predict(
      attempt_model,
      newdata = newdata,
      type = "response")
    tibble(
      age = a,
      attempt_num = cyc,
      predicted_p_mean = mean(newdata$predicted_p, na.rm = TRUE))
  })
})

# Bin groups below 30 and over 44 (due to low sample size)
attempt_predictions_kable <- attempt_predictions %>%
  ...
  group_by(age_table, attempt_num) %>%
  summarise(
    predicted_p_mean = mean(predicted_p_mean, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    age_table = factor(age_table, levels = c("<=30", as.character(31:43), ">=44"))
  ) %>%
  arrange(age_table, attempt_num)

kable(attempt_predictions_kable, digits = 3)

# Plot cycle-level probabilities per age
ggplot(attempt_predictions, aes(x = age, y = predicted_p_mean, color = factor(attempt_num))) +
